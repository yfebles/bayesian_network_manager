    ...
    public class BayesianGraph : Graph<BayesianNode>
    {
        ...
        private void SendMessageFromChildToParent(BayesianNode child, int parentIndex)
        {
            ...
            try
            {
                ...
                Console.WriteLine("Enviando mensaje desde  el hijo  {0} hasta  el padre {1}", child.Name, parent.Name);
                    ...
                    for (int childstate = 0; childstate < child.States; childstate++)
                    {
                        ...
                        foreach (var configuration in ParentRealizations(child, parentIndex, j))
                        {
                            otherParentsMessages =
                                child.ParentMessages.Select(configuration).AllExcept(parentIndex).Aggregate(1.0,
                                                                                                            (x, y) =>
                                                                                                            x*y);
                            parentRealization +=
                ...
                Console.WriteLine("El mensaje del hijo  {0} al padre  {1} es: {2}", child.Name, parent.Name,
                                  parent.ChildrenMessages[childIndex].Select(x => x.ToString() + ", ").Aggregate(
                                      (x, y) => x + y));
            }
            catch (Exception e)
            {
        ...
        {
            if (parentIndex == -1)
                return ParentRealizations(child.ParentMessages.Select(x => x.Length).ToArray(), 0,
        ...
        private void SendMessageFromParentToChild(BayesianNode parent, BayesianNode child)
        {
            ...
            try
            {
                Console.WriteLine("Enviando mensaje desde  el padre  {0} hasta  el hijo {1}", parent.Name, child.Name);
                ...
                for (int i = 0; i < parent.States; i++)
                {
                    double otherChildsMessages =
                        parent.ChildrenMessages.AllExcept(childrenIndex).Select(x => x[i]).Aggregate(1.0, (x, y) => x*y);
                    child.ParentMessages[parentIndex][i] = parent.Ro[i]*otherChildsMessages;
                }
                Console.WriteLine("El mensaje del padre  {0} al hijo  {1} es: {2}", parent.Name, child.Name,
                                  child.ParentMessages[parentIndex].Select(x => x.ToString() + ", ").Aggregate(
                                      (x, y) => x + y));
        ...
        private static void ComputeLambda(BayesianNode node)
        {
            ...
            {
                node.Lambda[i] = node.ChildrenMessages.Select(x => x[i]).Aggregate(1.0, (x, y) => x*y);
            }
        ...
        public void Work(object obj)
        {
            ...
            //while (!node.Finished)
            {
                if (Vertexes[Dictionary[node]].Count > 0 && Evidence[Dictionary[node]] == -1 && node.CanComputeLambda)
                {
                    Console.WriteLine("Calculando Lambda {0}", node.Name);
                    ComputeLambda(node);
                    Console.WriteLine("Lambda de  {0} es {1}", node.Name,
                                      node.Lambda.Select(x => x.ToString() + ",").Aggregate((x, y) => x + y));
                }

                if (Parents[Dictionary[node]].Count > 0 && Evidence[Dictionary[node]] == -1 && node.CanComputeRo)
                {
                    Console.WriteLine("Calculando Ro {0}", node.Name);
                    ComputeRo(node);
                    Console.WriteLine("Ro de  {0} es {1}", node.Name,
                                      node.Ro.Select(x => x.ToString() + ",").Aggregate((x, y) => x + y));
                ...
                for (int i = 0; i < Parents[Dictionary[node]].Count; i++)
                {
                    ...
                    if (node.CanSendParentMessages(i)) SendMessageFromChildToParent(node, i);
                }
                for (int i = 0; i < Vertexes[Dictionary[node]].Count; i++)
                {
                    //el nodo al que se le envia el mensaje no puede  estar en la evidencia
                    if (node.CanSendChildMessages(i))
                        SendMessageFromParentToChild(node, Vertexes[Dictionary[node]][i]);
                }
